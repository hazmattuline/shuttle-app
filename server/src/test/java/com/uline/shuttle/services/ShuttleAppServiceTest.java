...
public class ShuttleAppServiceTest {
    ...
    @InjectMocks
    ShuttleAppServiceImpl shuttleAppService;
    ...
    public void testChangeStatus() {
        Mockito.when(shuttleAppClient.changeStatus(Mockito.any(ShuttleRequest.class), Mockito.anyInt())).thenReturn(new ShuttleResponse());
        Assert.assertNotNull(shuttleAppService.changeStatus(new ShuttleRequest(), 1));
    }

    @Test
    public void testEnRoute() {
        Mockito.when(shuttleAppClient.enRoute( Mockito.anyInt(), Mockito.any(ShuttleRequest.class))).thenReturn(new ShuttleResponse());
        Assert.assertNotNull(shuttleAppService.enRoute( 1, new ShuttleRequest()));
    }

    @Test
    public void testGetDay() {
        Mockito.when(shuttleAppClient.getDay(Mockito.anyString(), Mockito.anyInt())).thenReturn(new ArrayList<>());
    ...
    public void testGetShuttlesStatus() {
        Mockito.when(shuttleAppClient.getShuttlesStatus(Mockito.anyString())).thenReturn(new ArrayList<>());
        Assert.assertNotNull(shuttleAppService.getShuttlesStatus( ""));
    }

    @Test
    public void testGetTrip() {
        Mockito.when(shuttleAppClient.getTrip(Mockito.anyString(), Mockito.anyInt())).thenReturn(new TripResponse());
        Assert.assertNotNull(shuttleAppService.getTrip( "", 1));
    }

    @Test
    public void testPostTrip() {
        Mockito.when(shuttleAppClient.postTrip(Mockito.any(TripRequest.class))).thenReturn(new TripResponse());
        Assert.assertNotNull(shuttleAppService.postTrip( new TripRequest()));
    }

    @Test
    public void testSubmitDay() {
        Mockito.when(shuttleAppClient.submitDay(Mockito.any(DayRequest.class))).thenReturn(new DayResponse());
    ...
    public void testSubmitNote() {
        Mockito.when(shuttleAppClient.submitNote(Mockito.any(NoteRequest.class))).thenReturn(new NoteResponse());
